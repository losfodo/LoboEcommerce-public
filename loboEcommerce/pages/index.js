...
//   const [currentPage, setCurrentPage] = useState(Number(router.query.page) || 1);//caso seja preciso mas nao é preciso
  //----------
  ...
  //---------------------------------
  const [scrollY, setScrollY] = useState(0);//para o botão de limpar os filtros nao vejo a diferença nele
...
//   console.log(scrollY, height);,, teste
  //---------------------------------
  return (
    <div className={styles.browse}>
      <div ref={headerRef} className={styles.browse__responsividade}>
      <MainSwiper />
        <div className={styles.browse__fixed}>
          <Header searchHandler={searchHandler} country={country} /> 
        </div>
      </div>
      <div className={styles.browse__container}>
        <div ref={el}>
          <div className={styles.browse__path}>Lobo Materiais de Construção / Home</div>
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => router.push("/")}//onClick={() => router.push("/browse")}
            >
              Limpar filtros  
              ({Object.keys(router.query).length})
            </button>
            {/* <div className={styles.browse__hidefilters}> */}
              <CategoryFilter
          ...
          <div className={styles.browse__store_products_wrap}>
            <div className={styles.browse__hidePrice}>
              <HeadingFilters
            ...
            <div className={styles.pagination}>
              <Pagination
                ...
                // defaultPage={Number(router.query.page) || 1} //page={currentPage}
...
export async function getServerSideProps(ctx) {
  const { query } = ctx;
  //-------------------------------------------------->
  const searchQuery = query.search || "";//campo de busca
  ...
  const pageSize = 9;//const pageSize = 50;
  ...
  const search =
    searchQuery && searchQuery !== ""
      ? {
          name: {//para pesquisar um produto pelo nome
            $regex: searchQuery,
            $options: "i",// Opção 'i' para tornar a pesquisa case-insensitive
          },
        }
      : {};
  const category =
  ...
  let productsDb = await Product.find({
    ...
    .limit(pageSize)
    .sort(sort)//.sort({ createdAt: -1 }) //.sort(sort)
    .lean();
...
//   console.log('styles', randomize(styles));
