...
handler.post(async (req, res) => {
  try {
    const { path } = req.body;
    let files = Object.values(req.files).flat();// flat() é um método de cópia. Isso não altera isso, mas retorna uma cópia superficial que contém os mesmos elementos que do array original.
    //res.json(files);//teste postman
    let images = [];
