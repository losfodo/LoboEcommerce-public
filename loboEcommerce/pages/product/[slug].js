...
export default function Product({ product, related }) {
  ...
  return (
    <>
      <Head>
        <title>{product.name}</title>{/*titulo da aba */}
      </Head>
      ...
      <div className={styles.product}>
        <div className={styles.product__container}>
          <div className={styles.path}>
            ...
            {product.subCategories.map((sub) => (//.map((sub, i) => ( <span key={i}>/{sub.name}</span>
...
export async function getServerSideProps(context) {
  ...
  const related = await ProductModel.find({ category: product.category._id }).lean();
  //------------
