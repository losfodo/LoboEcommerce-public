...
export default function AddReview({ product, setReviews }) {
  ...
  const [rating, setRating] = useState(0);//const [rating, setRating] = useState();
  ...
  const handleSubmit = async () => {
    ...
    if (msgs.length > 0) {
      ...
      if (images.length > 0) {
        ...
        // console.log('uploaded_images review', uploaded_images);
      }
    ...
    <div className={styles.reviews__add}>
      ...
      <div className={styles.reviews__add_wrap}>
        ...
        <Rating
          ...
          defaultValue={0}
          value={rating}
          onChange={(e, newValue) => setRating(newValue)}//onChange={(e) => setRating(e.target.value)}
          precision={0.5}
