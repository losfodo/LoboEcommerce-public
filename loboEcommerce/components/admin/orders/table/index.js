    ...
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        <TableCell>
          ...
          >
            {/* {open ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />} */}{open ? '⬆️' : '⬇️' }
          </IconButton>
        </TableCell>
              ...
              <Table size="small" aria-label="purchases">
                ...
                <TableBody>
                  {row.products.map((p, i) => (
                    <TableRow key={`${p._id}-${i}`}>{/* <TableRow key={p._id}> */}
                      <TableCell component="th" scope="row">
                        <img
                          src={p.image}
                          alt=""
...
// Row.propTypes = {//removendo o erro some
//   row: PropTypes.shape({
  ...
  return (
    <TableContainer component={Paper}>
      ...
      <Table aria-label="collapsible table" className={styles.table}>
        ...
        <TableBody>
          {rows.map((row, i) => (//<Row key={row.name} row={row} />
            <Row key={`${row._id}-${i}`} row={row} />
          ))}
        </TableBody>
      </Table>
    </TableContainer>
