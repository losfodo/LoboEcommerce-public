...
export default function Images({
  ...
  const [meta, field] = useField(props);//warnning para resolver
  const handleImages = (e) => {
    let files = Array.from(e.target.files);
    // console.log('files', files);
    files.forEach((img, i) => {
      // console.log('files forEach', files);
      if (files.length > 6) {//if (images.length == 6) {
        // console.log('files images.length', img);
        // console.log('files images.length', files);
        dispatch(
          showDialog({
